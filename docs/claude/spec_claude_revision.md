...
graph TB
    ...
    subgraph TIERS["任务验证（Judge Agent · 独立 Agent 实例）"]
        J["Judge Agent\n任务验证执行器"]
        C["Constraints 约束检查"]
        T["Todo 完成检查"]
        R["Requirements 满足检查"]
...
任务验证在认知构建之后执行，负责判断当前状态是否满足 `Picture`。检验在轮次结束后自动触发，是只读操作，不执行写操作。
...
sequenceDiagram
    ...
    MA->>FS: 轮次结束，写入 session.md 快照
    MA->>JA: 触发调用（传入 task_id）
...
**Judge Agent 的输入：** 调用时由宿主框架注入 `task_id`，Judge Agent 凭此从文件系统读取以下内容作为检验依据：

- `task.md`：`Picture`、`Requirements`、`Constraints`、Todo 列表
...
任务验证按顺序执行以下四层关卡。Tier 0/1/2 为客观检验条件，Judge Agent 依据文件内容自动判断，结论确定（PASS/FAIL），不涉及主观解读；Tier 3 为语义对齐关卡，需要 Judge Agent 运用语义理解能力，仅在满足触发条件时执行。

任何一个 Tier 失败，Judge Agent 立即停止后续 Tier 并输出 FAILED，不继续执行。

* **Tier 0: `Constraints` 约束检查。** Judge Agent 逐条比对 `Constraints` 与 session.md 最新快照及 gotchas.md 中的违反记录。若检测到逾越，记录违反事实（哪条 Constraint、在哪个轮次被逾越），输出 FAIL。Judge Agent 只报告事实，不做修复建议，修复决策由主 Agent 承担。
...
**示例：** 一个 `Picture` 为"用户无需输入密码即可登录"的 OAuth 任务。Tier 1 确认所有 Todo 已完成；Tier 2 验证"支持 Google OAuth"和"支持 GitHub OAuth"两条 Requirements 均达标；Tier 3 额外判断"实际登录流程中用户确实不会被要求输入密码"——这一判断无法通过比对 `code_progress` 字段完成，需要 Judge Agent 根据 `Picture` 语义和代码描述进行推断，属于语义对齐。
...
ABANDONED 由主 Agent 主动标记，与 Judge Agent 的检验结论无关——主 Agent 可在任意时刻废弃任务，不需要经过检验。
...
mem0ress 的全部设计，都在试图让这个问题变得可回答。任务模型将目标、进度和边界封装为可判断的整体；双重平面将认知状态和数据状态分开处理；快照协议确保每轮结束时的状态变更可以被追踪。每一个设计决策，都指向同一个问题：如何让 Agent 在任何时刻都能判断自己在哪里。
...
| `VERIFYING` | 任务验证进行中，瞬态，检验完成必须离开此状态 |
| `COMPLETED` | 目标达成，认知生命周期结束 |
| `ABANDONED` | 目标放弃，记录 Gotcha 经验 |
...
### Q: Judge Agent 是什么，它和主 Agent 是什么关系？
A: Judge Agent 是独立于主 Agent（Main Agent）之外的另一个 Agent 实例，二者的关系是**执行与检验的职责分离**。

主 Agent 负责执行任务——拆解 Todo、调用工具、写代码、推进进度。Judge Agent 只做一件事：在主 Agent 完成一个轮次后，独立地判断当前任务状态是否满足 PRC 模型的定义。
