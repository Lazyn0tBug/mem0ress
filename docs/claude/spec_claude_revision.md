...
**预设运行环境：** mem0ress 预设运行在**基于 Agent 的应用**中。这意味着运行时存在至少两个独立 Agent：执行任务的主 Agent（Main Agent）和专职执行任务检验的 Judge Agent。两者是独立的 Agent 实例，拥有各自的上下文窗口、系统提示和调用生命周期，通过文件系统（`judge.md`）进行异步协作，互不共享上下文。这一预设贯穿本规范所有设计决策，不适用于单 Agent 场景。
...
graph TB
    ...
    subgraph TIERS["任务检验（Judge Agent · 独立 Agent 实例）"]
        J["Judge Agent\n任务检验执行器"]
        C["Constraints 约束检查"]
        T["Todo 完成检查"]
        R["Requirements 满足检查"]
...
任务检验按顺序执行以下四层关卡。Tier 0/1/2 为客观检验条件，Judge Agent 依据文件内容自动判断，结论确定（PASS/FAIL），不涉及主观解读；Tier 3 为语义对齐关卡，需要 Judge Agent 运用语义理解能力，仅在满足触发条件时执行。
