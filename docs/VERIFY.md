...
| marker | 状态 | 说明 |
|--------|------|------|
| `[]` | 未确认（checked） | 尚未通过对话确认验证方式 |
| `[.]` | 已确认（checked） | 交互式验证方法已确认 |
| `[✓]` | 已完成（checked） | 验证通过 |
| `()` | 未确认（command） | 尚未通过对话确认验证方式 |
| `(.)` | 已确认（command） | 命令式验证方法已确认 |
| `(✓)` | 已完成（command） | 命令执行完成 |
| `{}` | 未确认（skip） | 尚未通过对话确认验证方式 |
| `{.}` | 已确认（skip） | 跳过，记录理由 |
| `{✓}` | 已完成（skip） | 跳过执行 |
...
| Constraints | 验证方式（怎么算满足） | 验证结论 |
| Requirements | 验证方式（怎么算满足） | 验证结论 |

**验证结果：未满足即不满足。** 不需要独立的违规状态——Constraint 验证失败时标记为未完成即可，不需要 `[×]` 这类违规中状态。

## 写入规则

1. **`[.]` / `(.)` / `{.}` 只能由人写入**：验证程序经人确认后，人将其标记为已确认
2. **Agent 可写验证结论**：执行验证后，将已确认的条目标记为 `[✓]` 或 `(\)`（skip）
