...
mem0ress 的设计基于四个认知科学洞察。这四个洞察回答的是"为什么这样思考"，而不是"用什么结构实现"——后者的答案在第三章。

四个洞察之间存在一条推导链。洞察一（记忆的目标属性）是起点——它否定了"存储优先"的记忆架构，指向"目标锚定"的方向。洞察二（完成标准的结构）在这一方向上进一步细化：目标锚定需要可判断的完成标准，方向上指向结构化。洞察三（认知单元的形态）和洞察四（认知的切分方式）从洞察二独立推导——结构化的完成标准需要一个载体，载体的内部结构和外部切分方式都从这一需求生长出来。
...
**洞察四：认知需要两个相互独立的切面——一个回答"做到了什么"，另一个回答"推进到哪了"。**

对任意认知单元，系统需要同时掌握两个不同维度的事实：

"做到了什么"是数据层面的——当前操作的是哪个版本的代码，文档处于什么状态。"推进到哪了"是执行层面的——任务进展到哪了，完成度如何，是否偏离目标。
...
**洞察三：任务是认知的完整单元——包含图景、需求、约束与执行进度，可在任意粒度下独立判断状态。**
...
**洞察四：任务的认知可以体现为两个平面——数据平面回答"做到了什么"，状态平面回答"推进到哪了"。**

对任意一个任务，观察者需要同时掌握两个不同层面的认知：
...
两个平面之所以需要分开，有认知效率的原因：如果每次获取状态都要同时加载数据版本和执行进度，认知负载翻倍；如果混合在一起，Agent 无法独立判断"我现在应该看数据还是看进度"。

状态平面在每次唤醒时强制挂载（因为它回答的是"我在哪"），数据平面则按需挂载——Agent 需要操作具体数据时才展开。两个平面都是时间切片——某一时刻的快照——而非组件或服务。

这种分离设计的认知收益是：Agent 可以独立思考进度问题或数据问题，而不需要同时处理两个维度的干扰。数据平面变化时，Agent 知道这是数据问题；任务状态变化时，Agent 知道这是进度问题。两种问题的处理策略不同，分开思考避免了认知混淆。

基于这两个平面的分离，Task 的整个执行循环围绕三个核心动作展开：**认知构建**、**任务检验**和**状态更新**。认知构建负责生成状态平面的快照，任务检验负责判断当前状态是否满足 Picture，状态更新负责将检验结果反映到 Task 状态。这三个动作在每个轮次结束后依次执行，构成完整的感知-判断-更新闭环。
