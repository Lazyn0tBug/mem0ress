...
mem0ress 将语义权威与状态管理分离。语义权威不属于状态平面，不得由状态平面做出"任务已完成"、"质量已足够"、"语义已对齐"等判断——这些判断由 Judge Agent 依据 Picture、Requirements 和 Constraints 做出，主 Agent 不得替代。

状态平面是 Judge Agent 感知、检验和判断结果的汇聚展示层。Judge Agent 的语义判断结论通过状态平面呈现，但不意味着状态平面拥有语义权威。

#### 3.4.1 认知边界

认知边界描述主 Agent 与 Judge Agent 的认知管辖范围划分：

- **主 Agent**：掌握执行状态、Todo 进度、代码操作记录（负责"做什么"）
- **Judge Agent**：掌握语义对齐判断权（负责"是否达成 Picture"）
- **边界**：主 Agent 不得替代 Judge 做出对齐判断，不得由状态平面做出"任务已完成"等语义结论
...
mem0ress 将漂移检测视为持续语义责任。

### 3.6 认知重构

mem0ress 的认知是前向构建的，不依赖历史回放。对齐恢复不需要完整 transcript，不需要穷尽推理历史，不需要完整执行时序——需要的只是当前 Picture、当前 Requirements、当前 Constraints 与当前证据之间的关系。
