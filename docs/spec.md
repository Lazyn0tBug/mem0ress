...
mem0ress 将语义权威与状态管理分离。Judge Agent 掌握语义对齐的判断权，不从事执行。语义权威不属于状态平面，不得由状态平面做出"任务已完成"、"质量已足够"、"语义已对齐"等判断——这些判断由 Judge Agent 依据 Picture、Requirements 和 Constraints 做出，主 Agent 不得替代。
