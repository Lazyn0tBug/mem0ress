...
### 1.3 核心解法：一切皆 Task，Task = PRC

mem0ress 的核心解法是**一切皆 Task**：将所有认知单元统一为同构的任务节点，每个任务由三个要素定义：

* **Picture（图景）：** 任务完成后的终极成功状态，回答"做成什么样"
* **Requirements（需求）：** 抵达图景的可验证硬性指标，回答"怎么证明成功了"
* **Constraints（约束）：** 执行时绝对不可逾越的底线，回答"什么绝对不能做"

三者共同构成判断未来动作是否偏离的绝对标准。Picture 位于最高层，指导 Requirements 制定，Requirements 反过来校验 Picture 的可达性，Constraints 作为冲突检测的锚点先行定义。

在 Task 之上，系统通过**状态平面**和**数据平面**两个时间切片，以极低的 Token 成本为 Agent 提供实时态势感知：状态平面告诉 Agent"此刻任务走到了哪"，数据平面告诉 Agent"此刻相关数据是什么版本"。两个切片都不是组件，而是任一时刻的快照，按需展开，不默认加载。

通过这种设计，系统在**任务创建、任务检验与认知构建**的严密心智循环中，帮助 Agent 快速发现并修正偏差。
