...
mem0ress 将语义权威与状态管理分离。状态平面积聚执行证据，不解释语义。Judge Agent 掌握语义对齐的判断权，不从事执行。

语义权威不属于状态平面，不得由状态平面做出"任务已完成"、"质量已足够"、"语义已对齐"等判断。Judge Agent 依据 Picture、Requirements 和 Constraints 做出语义对齐判断，主 Agent 不得替代。

状态平面提供结构化证据，记录执行事实，但不解释证据的语义含义。

mem0ress 要求语义判断不得委托给状态平面。
