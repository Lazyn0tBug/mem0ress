...
mem0ress 的设计基于四个相互独立的洞察。这些洞察源自记忆研究、认知科学和任务管理领域的交叉经验。它们共同构成了整个规范的认知科学基础，也为其他认知架构（如 MetaDev）提供了可独立引用的理论锚点。
...
这一特性在 AI Agent 的会话上下文中同样成立。当 Agent 切换任务目标时，上一轮对话中完整保留的中间结果对新目标可能完全无关。上下文污染（Context Collapse）的根本原因不在于上下文太长，而在于上下文没有被目标过滤——所有历史等权共存，有效信号被噪音淹没。

传统记忆系统的思路是"维护"：维护一个不断增长的向量库，通过相似性检索把相关内容捞出来。但这种方法有两个根本缺陷：第一，向量相似性不是语义相关性——"实现 OAuth 登录"和"密码加密存储"在向量空间中可能很近，但对一个具体的子任务来说，其中一个可能是噪音；第二，检索本身是向后看的，它只能找到"和现在相似的东西"，无法回答"我现在应该做什么"。

真正的问题不是"怎么保留相关信息"，而是"相关是相对于什么的"。答案是：相对于目标。目标决定了什么信息值得被提取，而不是信息本身决定了它是否相关。

这带来一个关键推论：如果目标明确，上下文视图是主动构建的，而非被动维持的。Agent 不需要在每个时刻都保留完整的会话历史——它只需要在任务开始时，根据目标动态构建当前状态的相关视图，并在执行过程中持续检验这个视图是否仍然有效。

这一洞察是 mem0ress 所有设计决策的起点。它从根本上否定了"存储优先"的记忆架构，转向"目标锚定的认知架构"。
...
**数据维度**（Data Plane）回答"当前操作的是什么版本的代码和文档"。它记录相关仓库的当前 commit ID 快照、长篇文档的版本指针。当 Agent 被重新唤醒时，它需要知道上次操作的是哪一行代码——而不是从会话历史中大海捞针。

**状态维度**（Status Plane）回答"当前任务推进到什么阶段了"。它聚合任务树结构、每个 Task 的 TODO 完成度、任务状态（CREATED / IN_PROGRESS / COMPLETED / ABANDONED）和偏差记录（Gotchas）。

两个维度之所以必须正交互斥，有认知效率的原因：如果每次获取状态都要同时加载数据版本和执行进度，认知负载翻倍；如果混合在一起，Agent 无法独立判断"我现在应该看数据还是看进度"。
...
**源自洞察一和二（2.1 + 2.2）：目标属性决定了认知以任务为中心；PRC 框架决定了认知以标准为导向。**
...
**源自洞察四（2.4）：状态平面与数据平面正交互斥，避免状态与数据之间的维度捆绑。**
