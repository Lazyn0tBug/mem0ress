...
* **Tier 0: Constraints 约束检查：** Tier 0 在每轮次结束后由系统自动触发。若有违反，尝试自动修复；若无法修复，按权限发起让度请求并同步等待外部响应（L1/L2 立即让度，L3/L4 失败后让度）。
...
Agent 通过让度机制实现人机协作——发起让度请求并同步等待外部响应，Agent 暂停执行直至收到确认。让度是 Agent 的主动行为，不是系统强制中断。
