...
不许留旧数据。运行时工作区（Agent 的认知空间）内产生新认知时，直接覆写旧认知——旧的不清理，新的也说不清。系统在覆写前做冲突检查，确保认知的确定性。持久化那一层另外说，遵循「不要存储要按需发现」原则。
...
mem0ress 的解法是：只做一件事，不加工数据。系统完全建立在"目录树 + 纯文本（Markdown/YAML）"之上，没有任何私有格式或隐藏状态。

好处有三个。首先，Agent 可以直接 `cat` 任意清单文件，看到的与系统存储的完全一致。没有任何 API 层对内容做截断或改写。其次，所有认知产物（task.md、Session、Gotchas）均在 Git 版本控制之下，任何变更均可追溯到具体的人和轮次。第三，目录深度表达依赖关系，文件名承载类型语义。Agent 不需要特殊工具就能理解和导航整个认知空间。

向量数据库 + 检索的模式把原始信息编码成高维向量，检索时再解码——这个过程本身就是信息损失。mem0ress 不一样：task.md、Session、Gotchas 两种人都能看懂。
