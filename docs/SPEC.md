...
**`/cap amend` 命令：** 在任意时刻修正 verify.md 条目。仅允许未确认条目（`[]` / `()` / `{}`）；已确认条目（`[.]` / `(.)` / `{.}`）执行前可 amend，执行后不可；已完成条目（`[\✓]` / `(\✓)` / `{\✓}`）不可修改，提示「已完成」；Constraint 的 `[×]`（违规中）可 amend（解决后转为 `[\✓]`）。amend 记录追加到 session.md（可审计）。
...
* **约束检查：** 检查 `Constraints` 是否被逾越。Constraint 违规不等于任务失败——违规时任务 SUSPEND（暂停），等待解决；解决后继续。人判断无法解决时可 override 继续（附理由）。
