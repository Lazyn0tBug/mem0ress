...
第五章描述任务执行循环的三个核心动作——认知构建、任务验证、状态更新——以及它们如何依赖快照协议在每一轮中运转。
...
在 CAP 中，Agent 在任意时刻只绑定一个活跃 Task。该 Agent 的认知平面只围绕当前 Task 的本地文档组装，包括 `task.md`、`session.md`、`gotchas.md` 和 `VERIFY.md`。任务树表达的是任务之间的分解关系、依赖关系和完成关系，而不是单个 Agent 的全局上下文窗口。
...
Task 是 Agent 的最小认知闭包。每个 Task 都拥有独立的 Picture、Requirements、Constraints、Todos、Session、Gotchas 和 verify 文件。Agent 对该 Task 的目标达成负责，而不是对整棵任务树负责。
...
三个核心动作按固定顺序执行：认知构建 → 任务验证 → 状态更新。认知构建先于任务验证，任务验证先于状态更新。
...
| 父任务可依赖 | 父任务不可依赖 |
|------------|-------------|
| child lifecycle state（COMPLETED / ABANDONED） | child todos / session / gotchas / verify |
...
* **VERIFY.md**：Verify Agent 检验记录 + 验证协议。与 Task 生命周期同步，任务创建时生成（空文件），检验后追加。包含所有 Requirement 和 Constraint 的验证方式定义，以及 Verify Agent 的检验结论。`[.]` / `(.)` / `{.}` = 已确认可执行，`[]` / `()` / `{}` = 待讨论。人确认后写入，是 Agent 执行检验的依据。
...
└── {task_id}/
    ...
    ├── gotchas.md    # 偏差记录（追加式）
    └── VERIFY.md     # Verify Agent 检验记录 + 验证协议
```

### 4.5 任务生命周期

任务从创建到结束经历五种状态：CREATED（模型已定义，所有 Todo 未开始）、IN_PROGRESS（至少有一个 Todo 已完成）、VERIFYING（任务验证进行中，瞬态）、COMPLETED（目标达成）、ABANDONED（目标放弃）。
...
**任务验证**在认知构建之后执行，判断当前状态是否满足 Picture。详见 §5.3。
...
Agent 不得替代 Verify Agent 做出语义对齐判断，不得由状态平面做出"任务已完成"、"质量已足够"、"语义已对齐"等判断——这些判断由 Verify Agent 依据 Picture、Requirements 和 Constraints 做出。

---

## 5. 逻辑与流程设计

Task 的执行循环围绕三个核心动作展开：认知构建、任务验证和状态更新。这三个动作在每个轮次结束后依次执行，构成完整的感知-判断-更新闭环——每轮次结束时 Agent 感知状态变更，构建新的认知快照，与既有目标对照，检验偏差并决策下一步。

### 5.1 执行循环

Task 的执行循环围绕三个核心动作展开：认知构建、任务验证和状态更新。这三个动作在每个轮次结束后依次执行，构成完整的感知-判断-更新闭环。
...
任务验证在认知构建之后执行，负责判断当前状态是否满足 `Picture`。验证在轮次结束后自动触发，是只读操作，不执行写操作。
...
**Tier 2 对话目标：** 生成确定性验证方法。验证方法本身可 replay。
**Tier 3 对话目标：** 对任务成果进行语言判断。结论是一次性的，不生成可 replay 的方法。
...
| `T3-N` | Picture | Tier 3 语义对齐结论，对任务成果进行语言判断 |
...
| 状态 | marker | 可作为执行依据 | 可 amend |
|------|--------|--------------|---------|
| 未确认 | `[]` / `()` / `{}` | ❌ | ✅ |
| 确认 | `[.]` / `(.)` / `{.}` | ✅ | ✅（执行前） |
| 已完成 | `[\✓]` / `(\✓)` / `{\✓}` | ✅ | ❌ |
| Constraint 违规 | `[×]` | ❌ | ✅（解决后转为 `[\✓]`） |
| Picture 结论 | `[.]` | ✅ | ✅（结论可更新，无"已完成"状态） |
...
**`/cap amend` 命令：** 在任意时刻修正 VERIFY.md 条目。

**amend 是唯一的 mutation 原语。** create = amend(mode=initial)——新任务的创建是 amend 的首次调用；已有条目的修正是 amend 的后续调用。

所有类型的条目均可 amend：
- 未确认条目（`[]` / `()` / `{}`）：直接 amend
- 确认条目（`[.]` / `(.)` / `{.}`）：执行前可 amend，执行后不可
- 已完成条目（`[\✓]` / `(\✓)` / `{\✓}`）：非持续性不可修改；持续性可退回 `[.]` 重新验证
- Constraint 的 `[×]`（违规中）：解决后转为 `[\✓]`
...
验证触发方式有三种（每 todo 完成 / 人主动 verify / 达到阈值），触发后统一执行同一路径：
...
```
每轮次：Tier 0（观察）→ 更新状态快照（不触发 Tier 3）
```

Tier 0 的作用是让状态平面保持最新，使人和 Agent 随时能看到 todo 进度。它不是 verify 的组成部分。
...
```yaml
verify:
  max_interval_rounds: 10  # 每 N 轮未主动 verify 则强制触发验证
```
...
**主 Agent 决策空间：** 主 Agent 读取 VERIFY.md 后可选择以下任意一条路径，无需外部批准：PASSED + 所有 Todo 完成后 → 调用 `complete_task` 进入 COMPLETED；PASSED + 发现新的 Todo → 继续执行；FAILED + 问题可修正 → 修正后重新触发检验；FAILED + 问题复杂 → 拆解子任务将问题分解；FAILED + 问题无解 → 调用 `abandon_task` 进入 ABANDONED；TIMEOUT → 重试检验或调用 `abandon_task`。主 Agent 不允许在 Verify Agent 未通过时调用 `complete_task`。

**ABANDONED 的处理义务：** 任务进入 ABANDONED 时，主 Agent 有以下义务：在 gotchas.md 追加废弃原因（`如何处理` 字段写"任务废弃"及原因）；确保所有直接子任务也处于终态（COMPLETED 或 ABANDONED）。ABANDONED 不需要经过 Verify Agent 检验，主 Agent 可在任意时刻主动废弃。
...
Verify Agent 语义角色由以下不变量定义：Verify Agent 负责建构任务的对齐状态，而非仅验证实现条件；Verify Agent 执行连续的对齐解释，评判当前任务状态是否仍然保持语义对齐，而非输出二元的通过/失败结果；Verify Agent 的语义解释构成状态平面的唯一来源，状态平面不由执行系统直接写入；Verify Agent 通过解释 Picture、Requirements、Constraints 与可用证据之间的关系来建构状态平面；对齐是连续的而非二元的，语义漂移可能通过局部优化、隐含假设或约束侵蚀逐渐发生。
...
| `Verify Agent` | 伴生组件，执行任务验证；Verify Agent 节点与 Task 节点并列平铺于同一任务目录下，VERIFY.md 是其物理文件 |

---

## 附录 B: FAQ

设计哲学 FAQ 已独立为 [docs/FAQ.md](docs/FAQ.md)，包含 8 个关于认知所有权、认知聚焦原则等核心哲学问题的解答。

用法相关问题请参考 [docs/tutorial.md](docs/tutorial.md)。
