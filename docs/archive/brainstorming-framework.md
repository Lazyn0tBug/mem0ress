...
mem0ress 中的冲突不是"需要协调的问题"，而是**任务拆分不够细的信号**。
...
**核心原则：冲突只能避免，无法解决。** 协调机制本身是错误的方向。正确做法是通过设计使冲突根本不发生。

**设计问题：** brainstorming 框架如何帮助识别"这个冲突意味着需要继续拆分"？
...
- 若有违反，报告违反事实，由主 Agent 决定是否修复及如何修复
- 若主 Agent 无法修复 → Agent 按权限决定：
  - L1/L2：立即让度给人（TODO 标记"待人确认"，spawn 协作任务）
  - L3/L4：尝试修复，失败则让度给人
- Judge Agent 只读不写，不参与任何数据修复

**Tier 1/2/3：纯检验，不做数据变更**

| 层级 | 检查内容 | 通过标准 | 是否有数据变更 |
|------|---------|---------|--------------|
| **Tier 0** | Constraints 是否满足 | 全部 Constraint 无违反 | 无（纯报告） |
...
## Open Questions
 ...
 1. **谁来定义锚点？** ✅ **已解决：** 通过 slash command 引导定义，人类和 AI 都走同一流程。锚点可随时更新。

 2. **Phase 2 (PRC 过滤) 的严格程度？** → ✅ **已解决：PRC 在想法产生时必须创建，无法提出有效 PRC 说明想法存在问题。** 更新 PRC = 更新任务本身，不存在独立的 PRC 过滤阶段。

 3. **冲突应该被解决还是被保留？** → ✅ **已解决：冲突只能避免，无法解决。继续拆分任务直到冲突消除，无法消除则让度给人。**
...
**Q: 为什么 mem0ress brainstorming 没有冲突协调机制？**

A: 冲突不能被协调。协调机制（锁、乐观锁、悲观锁、消息队列、共识协议）是试图在冲突发生后解决它，但更好的做法是通过设计使冲突根本不发生。

这来自 MetaDev 规范的核心洞察：**冲突只能避免，无法解决**。AI 的线性推理天然倾向于用问题分割代替跨层协调——当任务被正确拆分到合理粒度，每个子任务可以在独立的数据空间中线性推进。

所以冲突出现时，正确的处理方式是：**继续拆分任务直到冲突消除**。如果确实无法继续拆分，记录为"待人确认的边界情况"。

**Q: 为什么 brainstorming 必须有 Picture？**

A: 传统软件工程认为"测试用例通过即合格"，但这只能说明符合预定需求，不能认为是达到了目标。Picture 是利益相关者对"问题是否被真正解决"的最终判断。

AI Agent 具备语义理解能力，能够理解自然语言描绘的图景。这意味着可以用少量检验点配合语义理解来判定任务是否完成，不必为每个细节编写测试用例。

所有任务都需要 Picture，只是模糊程度不同。
