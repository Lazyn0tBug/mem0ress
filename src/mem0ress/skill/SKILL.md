...
| **Requirements** | What must be verifiable? (Agent derives verify_cmd for each requirement) |
...
**Requirements with verify_cmd**: Each Requirement includes a `verify_cmd` generated by the Agent. The user provides semantic intent (e.g., "白皮书需要至少3个架构图"), and the Agent derives the corresponding verification command (e.g., `python scripts/check_diagrams.py --min 3`).
