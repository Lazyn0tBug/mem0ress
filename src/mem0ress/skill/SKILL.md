...
**Constraint**: Completed non-persistent entries (`[\✓]` / `(\✓)` / `{\✓}` with `persistent: false`) cannot be amended — they are permanent. Persistent entries and unconfirmed/confirmed entries can be amended.
